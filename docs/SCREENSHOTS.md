...
This file is **not published** — it is excluded in `_config.yml`. It is the
instruction list for filling in the five image placeholders in the docs site.

Until a file exists, the page shows a broken image plus a visible "screenshot
placeholder" note. Delete the note paragraph from the page once the real image is
in place.
...
**Used on:** `docs/index.md` — the first image on the home page, directly under
"What it is".
...
**Used on:** `docs/getting-started/first-sync.md` — under "Turn on sending".
...
**Used on:** `docs/getting-started/first-sync.md` — under "Verify it landed".
...
**Used on:** `docs/deployment/tls-mtls.md` — under "Installing on the phone".
...
**Used on:** `docs/integrations/home-assistant.md` — after the discovery examples.
...
- Keep each file under roughly 400 KB. The pages are text-heavy and should stay
  fast on a phone.
- No retina `@2x` variants and no `srcset`. The theme's CSS scales images to the
  content width; a single sensible size is enough.
- If you later add more images, put them in the same directory and reference them
  as `../assets/screenshots/<name>.png` from a subdirectory page, or
  `assets/screenshots/<name>.png` from `index.md`.

...
type ChallengeResponse struct {
	...
	// DaysElapsed The days of the month that are behind us. Today is not one of them — it is counted in `days_remaining` instead, because there are still steps to be taken today, and `days_elapsed + days_remaining` is always `days_in_month`.
	//
	// This is the denominator of `steps_per_day`: dividing a month's total by 31 on the third of the month gives a "daily average" a third of the truth, and it does so most visibly at the start of every month.
	...
	// MeasuredDays How many entries of `days` carried a step measurement. The rest of `days` is the gap — the part of the month we know nothing about, which is exactly what a reader needs in order to know how much weight `steps` can bear.

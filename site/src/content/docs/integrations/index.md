...
| | Best when | Cost |
|---|---|---|
| **[MQTT with Home Assistant discovery](home-assistant/)** | You already run an MQTT broker | Home Assistant creates the entities itself; nothing to maintain on either side |
| **[REST polling](rest-fallback/)** | You do not want a broker | Hand-written YAML, no discovery, and you own the polling schedule |
...
:::danger[Daily summaries go out. Raw samples do not.]
...
| **Sync freshness** | Every 15 minutes | **The one alert that matters** — see [below](home-assistant/#the-alert-that-matters) |
